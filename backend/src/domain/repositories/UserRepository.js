...
export class UserRepository {
  ...
  async update(id, { nome, email, nivel_acesso, ativo }) {
    const [result] = await this.database.execute(
      ...
      [nome, email, nivel_acesso, ativo === false ? 0 : 1, id, process.env.ROOT_ADMIN_EMAIL, email?.toLowerCase() || '', process.env.ROOT_ADMIN_EMAIL]
    );
  ...
  async updateAccess(id, { nivel_acesso, ativo }) {
    ...
    const [result] = await this.database.execute(
      ...
      [...values, id, process.env.ROOT_ADMIN_EMAIL]
    );
    return result.affectedRows > 0;
  }
  ...
  async delete(id) {
    const [result] = await this.database.execute(
      ...
      [id, process.env.ROOT_ADMIN_EMAIL]
    );
    return result.affectedRows > 0;
  }
  ...
  async updatePassword(id, newPassword) {
    ...
    const [result] = await this.database.execute(
      ...
      [hashedPassword, id, process.env.ROOT_ADMIN_EMAIL]
    );
    return result.affectedRows > 0;
  }
}