...
export const findOrdersByUserId = async (usuario_id, { page = 1, limit = 20 } = {}) => {
    ...
    const [rows] = await pool.execute(`SELECT * FROM pedidos WHERE usuario_id = ? ORDER BY criado_em DESC LIMIT ? OFFSET ?`, [usuario_id, limit, offset]);
    const [countRows] = await pool.execute('SELECT COUNT(*) AS total FROM pedidos WHERE usuario_id = ?', [usuario_id]);
...
export const findAllOrders = async ({ page = 1, limit = 20 } = {}) => {
    const offset = (page - 1) * limit;
    const [rows] = await pool.execute(`SELECT p.*, u.nome as usuario_nome, u.email as usuario_email FROM pedidos p
     LEFT JOIN usuarios u ON p.usuario_id = u.id
     ORDER BY p.criado_em DESC LIMIT ? OFFSET ?`, [limit, offset]);
...
export const updateOrderStatus = async (id, { status_pagamento, metodo_pagamento }) => {
    ...
    if (updates.length === 0)
        return false;
    const [result] = await pool.execute(`UPDATE pedidos SET ${updates.join(', ')} WHERE id = ?`, [...values, id]);
