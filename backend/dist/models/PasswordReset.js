...
export const createPasswordResetToken = async (email) => {
    const [users] = await pool.execute('SELECT id, email FROM usuarios WHERE email = ? LIMIT 1', [email]);
    const user = users[0];
    ...
    await pool.execute('INSERT INTO password_resets (usuario_id, token_hash, expira_em) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 30 MINUTE))', [user.id, tokenHash]);
    return token;
};
export const resetPasswordWithToken = async (token, newPassword) => {
    ...
    try {
        ...
        const [resets] = await connection.execute(`SELECT pr.id, pr.usuario_id, u.email
