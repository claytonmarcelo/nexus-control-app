...
export const getUserPermissionsById = async (req, res) => {
    try {
        ...
        const [userRows] = await pool.execute('SELECT id, nome, email, nivel_acesso FROM usuarios WHERE id = ?', [userId]);
...
export const updateUserPermissions = async (req, res) => {
    try {
        ...
        const [userRows] = await pool.execute('SELECT id, nome, email FROM usuarios WHERE id = ?', [userId]);
        if (userRows.length === 0) {
            return sendError(res, 'Usuário não encontrado', 404);
        }
