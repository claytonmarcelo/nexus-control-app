...
export const updateCurriculo = async (req, res) => {
    try {
        ...
        const updatedData = typeof updatedRows[0].dados === 'string' ? JSON.parse(updatedRows[0].dados) : updatedRows[0].dados;
        sendSuccess(res, { curriculo: updatedData, updated_at: updatedRows[0].updated_at }, 'Currículo atualizado com sucesso');
    }
    catch (error) {
