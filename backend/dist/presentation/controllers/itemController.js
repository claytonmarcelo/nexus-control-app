import { createItem, findAllItems, findItemById, findItemsByUser, updateItem, deleteItem, canUserModifyItem, } from '../../infrastructure/Item.js';
...
export const getMyNegotiations = async (req, res) => {
    try {
        sendSuccess(res, { negotiations: await findNegotiationsByUser(req.user.id) }, 'Suas simulações');
    }
    catch (error) {
        sendError(res, 'Erro interno do servidor', 500);
    }
...
export const getById = async (req, res) => {
    try {
        ...
        if (!item)
            return sendError(res, 'Item não encontrado', 404);
...
export const update = async (req, res) => {
    try {
        ...
        if (!canModify)
            return sendError(res, 'Sem permissão para modificar este item', 403);
...
export const remove = async (req, res) => {
    try {
        ...
        const canModify = await canUserModifyItem(id, req.user.id, req.user.nivel_acesso);
        if (!canModify)
            return sendError(res, 'Sem permissão para excluir este item', 403);
