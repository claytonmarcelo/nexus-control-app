...
export const getUserPermissionsById = async (req, res) => {
    try {
        ...
        const [userRows] = await pool.execute('SELECT id, nome, email, nivel_acesso, ativo FROM usuarios WHERE id = ?', [userId]);
...
export const updateUserPermissions = async (req, res) => {
    try {
        ...
        const [updatedRows] = await pool.execute('SELECT id, nome, email, nivel_acesso, ativo FROM usuarios WHERE id = ?', [userId]);
...
export const getAllUsersWithPermissions = async (req, res) => {
    try {
        ...
        const usersWithPermissions = await Promise.all(paginatedUsers.map(async (user) => {
...
export const getDashboardStats = async (req, res) => {
    try {
        ...
        const [ordersStats] = await pool.execute(`SELECT 
        COUNT(*) as total,
        SUM(total) as revenue,
        status_pagamento,
        COUNT(CASE WHEN status_pagamento = 'confirmado' THEN 1 END) as confirmados
      FROM pedidos GROUP BY status_pagamento`);
        const [userStats] = await pool.execute(`SELECT nivel_acesso, COUNT(*) as count FROM usuarios GROUP BY nivel_acesso`);
        sendSuccess(res, {
